...
export function assembleHealth(
  ...
  if (!s.settingsFound || s.installedEvents.length === 0) {
    status = "down";
    detail = "Session-tracking hooks aren't installed — states fall back to transcript inference (~8s lag, exits can stick).";
  } else if (s.missingRequired.length > 0) {
    status = "degraded";
    detail = `Session-tracking hooks partially installed — missing ${s.missingRequired.join(", ")}.`;
